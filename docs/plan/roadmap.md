...
| **S-150** | Translation + dubbing (TTS / voice cloning) | S-140, S-110 consent precondition | 🟡 planned 2026-08-13 — T0, T1a, T1b, T1c-i, T1c-ii, the T2a seam extraction, T2b-i, and all three T2b-ii delivery-repository children are complete. The slice now has product-code domain types, four artifact kinds, per-localization statuses, exact current-generation pointer/claim storage, fail-closed repositories, durable per-target dispatch identities, atomic claim/outbox persistence, and a guarded pending-to-enqueue-failed transition. `T2c` (versioned jobs plus outbox-backed fan-out) is next after a fresh RRI and approval. Queue, worker, and TTS/runtime work remain pending; T8 tracks non-blocking future voice-consent hardening. | `docs/plan/s-150-translation-dubbing.md`, `docs/tasks/s-150-translation-dubbing.md`, `workers/translation-worker-py`, `workers/tts-worker-py` |
