...
- **ADR candidates for product-layer phases (X22/X23/X24).** These are now all closed:
  ...
  - **X-S-160-3:** open — S-140 now creates review tasks from real subtitle readiness, and `S-140-T5b-a` added nullable `review_tasks.subtitle_artifact_id` identity storage. The remaining downstream ownership wiring and any explicit artifact-version semantics were deferred beyond S-140 closeout; a future dedicated follow-up (later S-150/S-170-scoped work or a standalone task) must own that work explicitly.
