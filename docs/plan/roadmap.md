...
| **S-130** | Processing / ASR (transcription) | S-100 target-language intent, S-120 | 🟡 all work complete locally 2026-07-19 — T1–T5 complete; domain/migration/repository, preparation-ready enqueue, ASR worker dispatch + readiness gating, Python `faster-whisper` worker, and canonical BDD/docs sync are delivered. The final green roadmap marker is deferred until the updated plan/task evidence files are committed, per `check-roadmap-drift`. | `docs/plan/s-130-asr-transcription.md`, `docs/tasks/s-130-asr-transcription.md` |
