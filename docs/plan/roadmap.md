...
| **S-150** | Translation + dubbing (TTS / voice cloning) | S-140, S-110 consent precondition | 🟡 planned 2026-08-15 — T0, T1a, T1b, T1c-i, T1c-ii, the T2a seam extraction, T2b-i, all three T2b-ii delivery-repository children, `T2c-i`, `T2c-ii`, `T2c-iii`, `T2c-iv-a0`, and `T2c-iv-b` are complete; `T2c-iv-a` has its contract cutover implemented and reviewed but stays `[~]` pending the still-open `T2c-vi-a` runtime cut. The slice now has product-code domain types, four artifact kinds, per-localization statuses, exact current-generation pointer/claim storage, fail-closed repositories, durable per-target dispatch identities, atomic claim/outbox persistence, guarded pending-to-enqueue-failed and pending-to-acknowledged transitions, versioned subtitle/translation job contracts with deterministic initial UUIDv5 identity, exact persisted Subtitle replay resolution, a local-sized 286-line job-contract module, and a subtitle producer that builds `SubtitleJob` with only the route-free asset/project constructor (`T2c-iv-b`, RRI 32, zero-diff formal closure of the 2026-08-15 workspace-compile compatibility patch, local-first `qwen3.6:35b-a3b` run + Gemma phase-1/phase-2 review). On 2026-08-13 the owner rejected the unused legacy-review compatibility path; the resulting T2c-iv surface scored RRI 63 and is decomposed into `T2c-iv-a0/a/b/c`, followed by the narrowed `T2c-v` Redis adapter and decomposed `T2c-vi` runtime/cleanup cutover. `T2c-iv-c` (durable localization fan-out service, provisional RRI 49 Med-high) is next, followed by `T2c-v` and the `T2c-vi-a/b` runtime cutover. Queue, worker, and TTS/runtime work remain pending; T8 tracks non-blocking future voice-consent hardening. | `docs/plan/s-150-translation-dubbing.md`, `docs/tasks/s-150-translation-dubbing.md`, `workers/translation-worker-py`, `workers/tts-worker-py` |
