...
| **S-150** | Translation + dubbing (TTS / voice cloning) | S-140, S-110 consent precondition | 🟡 planned 2026-08-13 — T0, T1a, T1b, T1c-i, T1c-ii, the T2a seam extraction, T2b-i, all three T2b-ii delivery-repository children, `T2c-i`, `T2c-ii`, `T2c-iii`, `T2c-iv-a0`, and `T2c-iv-a` are complete. The slice now has product-code domain types, four artifact kinds, per-localization statuses, exact current-generation pointer/claim storage, fail-closed repositories, durable per-target dispatch identities, atomic claim/outbox persistence, guarded pending-to-enqueue-failed and pending-to-acknowledged transitions, versioned subtitle/translation job contracts with deterministic initial UUIDv5 identity, exact persisted Subtitle replay resolution, and a local-sized 286-line job-contract module now stripped of the legacy post-ready-route mechanism. On 2026-08-13 the owner rejected the unused legacy-review compatibility path; the resulting T2c-iv surface scored RRI 63 and is decomposed into `T2c-iv-a0/a/b/c`, followed by the narrowed `T2c-v` Redis adapter and decomposed `T2c-vi` runtime/cleanup cutover. `T2c-iv-a`'s first phase-1 review (2026-08-15) returned BLOCKED on a scope gap; the ledger was corrected the same day (scope narrowed to keep `target_language` untouched and deferred to `T2c-vi-a`, `crates/jobs/src/lib.rs` added for its dead re-export) and the re-review passed at RRI 40 (`docs/audit/s-150-t2c-iv-a-phase1-review-v2.json`). The owner approved the same day; local-first routing (`qwen3.6:35b-a3b`) was unavailable in the executing session (egress blocks Ollama), so the primary agent implemented directly as the operational cloud-takeover route. `SubtitlePostReadyRoute`, the `post_ready_route` field, and `new_s150_localization` are removed; `target_language` and the 3-arg `new()` constructor are unchanged. D14 same-provider-degraded phase-2 review passed with no findings (`docs/audit/gemma-evidence/S-150-T2c-iv-a-phase2.json`); owner final verification recorded 2026-08-15 (commit `2223856`). `T2c-iv-b` (subtitle producer cutover) is next. Queue, worker, and TTS/runtime work remain pending; T8 tracks non-blocking future voice-consent hardening. | `docs/plan/s-150-translation-dubbing.md`, `docs/tasks/s-150-translation-dubbing.md`, `workers/translation-worker-py`, `workers/tts-worker-py` |
