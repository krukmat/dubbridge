...
| **S-140** | Subtitle generation | S-130 | 🟡 in progress — T0/T1a/T1b-i/T1b-ii/T1c/T1d/T2a/T2b-i/T2b-ii/T3a/T3b/T5a/T3c-i done (2026-07-22: T2b-i extraction seam + transcription atomic-claim race fix; T2b-ii direct subtitle enqueue hook, deterministic first-target routing, subtitle atomic claim, and worker-runner refactor that reduced `main.rs` to a 44-line startup entrypoint; T3a/T3b segmentation provider + handler/readiness; T5a ADR-030 review-task enqueue; T3c-i Redis-backed job queue 2026-07-26); T5a is `[x] Done (file-level)` on branch `local/s-140-t5a`, **not yet merged to `main`** — T6 (BDD feature + docs sync) stays blocked until that merge lands; T3c-ii/T3c-iii/T3c-iv (consumer-loop wiring, CI self-skip gap, apalis-redis future-incompatibility) are filed but off the critical path | `docs/plan/s-140-subtitle-generation.md`, `docs/tasks/s-140-subtitle-generation.md` |
