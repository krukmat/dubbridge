...
| **S-150** | Translation + dubbing (TTS / voice cloning) | S-140, S-110 consent precondition | 🟡 planned 2026-08-02 — T0, T1a, T1b, T1c-i, and T1c-ii complete; ratified slice boundaries now have matching product-code domain types, the verified migration layer for the four S-150 artifact kinds, per-localization translation/dubbing statuses, exact current-generation pointer/claim storage, and the fail-closed translation/dubbing repository layer that consumes that schema. The original `T1c` rerun scored `RRI 56` and was decomposed into `T1c-i` and `T1c-ii`, both now complete. `T2` is the next executable child; queue, worker, and TTS/runtime work remain pending, and T8 still tracks non-blocking future voice-consent hardening. | `docs/plan/s-150-translation-dubbing.md`, `docs/tasks/s-150-translation-dubbing.md`, `workers/translation-worker-py`, `workers/tts-worker-py` |
