...
| **MVP0-P2P** | P2P-first invited playback: maintainable mobile/Bare runtime foundation, isolated replication proof, encrypted publication, invite control plane, verified local package playback, and no-HTTP-fallback certification | S-120, S-125, S-127, S-160; accepted ADR-043 + separately approved P1 children before source work; separate audience-delivery ADR before P2 | 🟡 P0 closed — Android-only Bare worklet build/runtime proof passed. Revised P1 (RRI 94) and ADR-043 were approved on 2026-08-27. P1.F1 was then separately approved, implemented, and closed PASS after owner verification: deterministic `bare-pack`, versioned `bare-rpc`, handshake/capability validation, bounded ping, typed/redacted failures, fatal receipts, suspend/resume, timeout and clean shutdown pass 12 focused tests, 91.52% line coverage, and the full 252-test mobile suite. P1.F2 is the next gated child and may be prepared/presented, but its source execution still needs a current RRI/card/approval. The temporary P0 probe/bridge scaffold stays unchanged as a characterization oracle until P1.F3a; iPhone/iOS remains deferred and no product P2P runtime or network activity is active. | `docs/plan/mvp0-p2p-first.md`, `docs/tasks/mvp0-p2p-first.md`, `docs/plan/mvp0-p2p-p1-replication.md`, `docs/tasks/mvp0-p2p-p1-replication.md`, `docs/adr/ADR-043-mobile-p2p-runtime-ownership-and-proof-isolation.md`, `docs/audit/mvp0-p2p-p1-f1-implementation.md`, `p2p-mvp/` |
