...
| **S-150** | Translation + dubbing (TTS / voice cloning) | S-140, S-110 consent precondition | 🟡 planned 2026-08-12 — T0, T1a, T1b, T1c-i, T1c-ii, the T2a seam extraction, and T2b-i (translation-dispatch outbox migration) are complete; the slice has product-code domain types, four artifact kinds, per-localization statuses, exact current-generation pointer/claim storage, fail-closed repositories, and a durable per-target dispatch identity table. The former fan-out parent `T2` was decomposed into `T2b-i` (done), `T2b-ii` (repository and exact asset–project–target binding, next), and `T2c` (versioned jobs plus outbox-backed fan-out). Queue, worker, and TTS/runtime work remain pending; T8 tracks non-blocking future voice-consent hardening. | `docs/plan/s-150-translation-dubbing.md`, `docs/tasks/s-150-translation-dubbing.md`, `workers/translation-worker-py`, `workers/tts-worker-py` |
