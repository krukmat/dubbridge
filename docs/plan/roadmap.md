...
| **S-200** | Mobile credential login with backend-issued JWT (FenixCRM parity) | S-000, S-040, S-050 (re-architects their auth) | 📄 Planned 2026-06-17 — design package only (ADR-031 Proposed); inverts ADR-023/024, amends ADR-029; initiative RRI 109 (Excessive) so ADR + risk analysis + decomposition precede any implementation | `docs/plan/s-200-mobile-jwt-credential-auth.md`, `docs/tasks/s-200-mobile-jwt-credential-auth.md` (ADR-031) |
