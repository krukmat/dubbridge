...
| **S-140** | Subtitle generation | S-130 | 🟡 in progress — T0/T1a/T1b-i/T1b-ii/T1c/T1d/T2a/T2b-i/T2b-ii/T3a/T3b/T3c-i/T3c-ii/T3c-iii/T5a/T5b-a/T6 complete; canonical BDD + roadmap/plan/task sync landed 2026-07-30, and T5a is merged to `main` via `620d675`. `T3c-ii` closed via owner-waived phase-2 review; `T3c-iii` closed 2026-07-30 with explicit Redis CI gating and review evidence; `T3c-iv` remains deferred and `X-S-160-3` still needs downstream review-task wiring/version follow-up beyond the new artifact-identity column | `docs/plan/s-140-subtitle-generation.md`, `docs/tasks/s-140-subtitle-generation.md` |
...
- **ADR candidates for product-layer phases (X22/X23/X24).** These are now all closed:
  ...
  - **X-S-160-3:** open — S-140 now creates review tasks from real subtitle readiness, and `S-140-T5b-a` added nullable `review_tasks.subtitle_artifact_id` identity storage. Remaining gap: downstream repo/enqueue ownership wiring and any explicit artifact-version semantics are still unresolved. Gate: a follow-up after `T5b-a` (`S-140-T5b` remainder or later S-150-scoped work).
