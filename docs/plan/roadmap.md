...
| **MVP0-P2P** | P2P-first invited playback: encrypted publication, invite/claim, verified mobile sync, loopback playback, product surface, and no-HTTP-fallback certification | S-120, S-125, S-127, S-160; ADR-043 and ADR-044 Accepted; S-230 P2P deployment/RC gates | 🟡 P0 and P1 closed. P2.T0 PASS; P2.T1a-T1f Done and owner-approved as P2.T1; P2.C0 PASS on 2026-09-06. C0 froze `p2p-manifest-v1`, `p2p-aad-v1`, K1 custody, `availability-publication-v1`, P2 audit correlation, and `p2p-ready-descriptor-v1`. P2.T2 leaf-by-leaf: `T2a-i`/`T2a-ii-1`/`T2a-ii-2a`/`T2a-ii-2b`/`T2b`/`T2c`/`T2d` are Done (T2d closed 2026-09-07 — generate-once CK + versioned KEK wrap/unwrap + zeroization, RRI 28 Moderate, `crates/p2p/src/key_wrap.rs`); `T2e`-`T2g` remain planned. P3-P7 remain pending. T6p-a is deferred until S-230 T7local, T7c, and MVP0-P2P P2-P6 are PASS —
...
- **MVP0-P2P `P3`–`P7`** still need detailed per-phase executable plan files.
  ...
  of one-liners — so P3–P7 can be analyzed and presented without opening the
  untracked external `p2p-mvp/` package. **Still missing:** a detailed plan
  file per P3-P7 phase. Each remaining executable phase still needs
