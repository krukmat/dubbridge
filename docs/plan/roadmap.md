...
| **S-150** | Translation + dubbing (TTS / voice cloning) | S-140, S-110 consent precondition | 🟡 planned 2026-08-09 — T0, T1a, T1b, T1c-i, T1c-ii, and the T2a seam extraction are complete; the slice has product-code domain types, four artifact kinds, per-localization statuses, exact current-generation pointer/claim storage, and fail-closed repositories. The former fan-out parent `T2` was decomposed into `T2b-i` (durable dispatch-outbox migration), `T2b-ii` (repository and exact asset–project–target binding), and `T2c` (versioned jobs plus outbox-backed fan-out). Queue, worker, and TTS/runtime work remain pending; T8 tracks non-blocking future voice-consent hardening. | `docs/plan/s-150-translation-dubbing.md`, `docs/tasks/s-150-translation-dubbing.md`, `workers/translation-worker-py`, `workers/tts-worker-py` |
