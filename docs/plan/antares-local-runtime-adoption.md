...
flowchart TB
    ...
    T3d --> T4
    DEC --> T4
    T4 --> T5["T5 [x] Done 2026-08-06\npromote (owner-directed,\nno calibration/pilot evidence)"]
```
...
| E | T4 pilot, then T5 | **T4 implemented 2026-08-06** (RRI 51 Med-high; ADR-038 routed `CLOUD_REQUIRED` on both Qwen27 refinement and the primary receipt; implemented by Claude Code). **T5 closed 2026-08-06**: owner-directed **promote** of the existing advisory-only, three-touchpoint scope, decided without waiting for the 30-day live pilot window or a calibration run against the fixed thresholds (File F1 >= 0.30, TN rate >= 0.70) — see `docs/tasks/antares-security-specialist-advisor.md` § T5 Decision record for the explicit deviation statement. | existing slice gates |
