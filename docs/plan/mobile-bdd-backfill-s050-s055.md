...
> **Status:** Closed. Authored 2026-06-12; canonical BDD home consolidated into
> `docs/bdd/` on 2026-06-21.
...
- Normalize the canonical BDD home so future mobile-only slices can coexist
  without turning `docs/bdd/README.md` into a single-slice document.

## Scope decisions

| Decision | Choice |
|---|---|
| BDD home for mobile-only slices | Superseded 2026-06-21 by BDC-T1; canonical `.feature` specs now live under `docs/bdd/` |
| Existing `docs/bdd/README.md` | Expand the canonical BDD index with the mobile retrospective specs |
...
| Layer | Path | Change |
|---|---|---|
| BDD | `docs/bdd/README.md` | Convert from S-060-only guide into multi-slice mobile BDD index |
| BDD | `docs/bdd/s-050-mobile-client.feature` (new) | Behavioral Gherkin spec for the shipped mobile client |
| BDD | `docs/bdd/s-055-maestro-suite.feature` (new) | Behavioral Gherkin spec for the shipped two-phase screenshot suite |
...
### D2 — Historical split-home decision was later superseded

Cross-surface slices (`S-100`, `S-110`) use `docs/bdd/`. Mobile-only slices should
have lived under `mobile/bdd/`, next to their Maestro flows and mobile-facing
mapping. That split-home decision was later superseded by
`docs/tasks/bdd-home-consolidation.md` on 2026-06-21, which consolidated all
canonical BDD specs into `docs/bdd/`.
...
precedent, and all three slices now keep their canonical `.feature` files in
`docs/bdd/`.
...
- `S-060` is the precedent for mobile-first BDD mapped to Maestro-owned evidence.
