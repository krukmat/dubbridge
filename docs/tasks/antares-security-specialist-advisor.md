...
**Forward reference (informational, does not reopen T6):** the manual-mirror
gap this task hit — having to hand-copy edits from `AGENT_WORKFLOW_GUIDE.md`
into `AGENTS.override.md` because the latter was a hand-maintained duplicate
with no drift detection — is now closed by task AOS1
(`docs/tasks/agents-override-sync.md`), completed 2026-08-07. `AGENTS.override.md`
is now generated by `scripts/generate-agents-override.py` from `AGENTS.md` +
`AGENT_WORKFLOW_GUIDE.md` + `HITL_AUTONOMY_POLICY.md`, and a drift-check
function in `scripts/check-doc-consistency.sh` (run by `make qa-docs`) fails
closed if the file goes stale relative to its sources. Future edits to
`AGENT_WORKFLOW_GUIDE.md` no longer require a T6-style manual mirroring step.
