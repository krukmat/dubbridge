...
| T6p-b | P2P Compose/config/secrets/private-network wiring | config/ops | `apps/availability-node/Dockerfile`; `infra/production/docker-compose.yml`; `.env.example`; `config/README.md`; image-contract regression guard | T6p-a PASS | [~] REOPENED/FIXED 2026-09-26 — ARM64 runtime exposed missing `libatomic.so.1`; `libatomic1` added and guarded; awaiting T6p-c recertification |
| T6p-c | Local P2P deployment-contract evidence | operational/evidence | `infra/production/p2p/preflight.sh`; `scripts/test-production-images.sh` availability case; evidence/status docs | T6p-b amended image ready for recertification | [~] IN PROGRESS 2026-09-26 — prior image evidence invalidated after ARM64 `libatomic.so.1` failure; rebuild/runtime recertification required |
| T6p-d | Deploy backend ciphertext publication + durable P2P_READY smoke on DO | operational | TBD exact-path | T6p-c PASS | [ ] BLOCKED — requires T6p-c runtime PASS |
