...
| 2026-09-13 (owner-directed) | RRI 56+ (Complex+) phase-1/phase-2 primary reviewer | Cross-vendor peer (codex/claude by caller identity) | `gpt-oss:20b` at the Complex review profile (`num_ctx=49152`, `num_predict=8192`, `think=high`, `temperature=1.0`, `top_p=1.0`); cross-vendor peer becomes intermediate fallback, D14 remains final fallback | Owner directive during `P2.T3c-Integ` after excluding Codex from this session ("evita volver a usar codex"); explicit instruction to make gpt-oss primary for the Complex band with Codex retained only as fallback, not a capability/availability finding about Codex |
| 2026-09-14 (owner-approved, evidence-based) | RRI 56+ (Complex+) Complex review profile reasoning level | `num_predict=8192`, `think=high` | `num_predict=10240`, `think=medium` (same primary model, same fallback chain) | Four real `high`-reasoning review attempts (`P2.T3c-Integ` ×2, `P2.T4b` ×2) all returned `done_reason: length` with empty visible content — the budget was exhausted on hidden reasoning before a verdict; the one completed attempt in the same corpus ran at `medium`/`10240` and returned a valid `PASS`. See `docs/audit/local-execution-routing-evidence-2026-09-14.md` § "`gpt-oss:20b` Complex-review profile" |
...
| 2026-09-05 | Honest Low-band maximization before presentation | Permanent owner directive: score the coherent parent, then maximize truthful, independently verifiable RRI 0–25 / Effort S leaves so local development carries bounded work within hardware limits; parent approval/review gates and inseparable residue retain their real bands | ADR-038 Amendment 5; Guide § Honest Low-band maximization before presentation; RRI § Honest Low-band maximization is the default first pass; HITL § Parent envelope and honest Low-band maximization |
| 2026-09-13 | RRI 56+ primary reviewer inverted from cross-vendor-peer-first to gpt-oss-first | Permanent owner directive (not scoped to one task): `gpt-oss:20b` at the Complex profile is now the Step 1-C/Phase 1 and 2 primary for RRI 56+; the cross-vendor peer is the intermediate fallback, D14 remains final. Applies to both phases per explicit owner confirmation | Guide § Band-routed peer review (two phases); Guide § Step 1-C; HITL § Band-routed peer review; binding row above |
