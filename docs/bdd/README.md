...
| S140_HP2 | Successful subtitle processing persists one subtitle artifact, marks readiness, and fans out one localization job per configured target language | S-140-T3b, S-150-T2c-vi-a | `apps/worker-runner/src/subtitle_runtime_tests.rs::process_subtitle_job_marks_ready_and_stores_artifact_on_success`; `apps/worker-runner/src/runner_topology_tests.rs::redis_monitor_wires_preparation_transcription_and_subtitle_workers` | — | HP |
...
| S140_EC3 | One translation-queue enqueue failure marks only that dispatch failed; subtitle job still succeeds and no legacy review row is created | S-150-T2c-vi-a | `apps/worker-runner/src/subtitle_runtime_tests.rs::process_subtitle_job_marks_dispatch_enqueue_failed_when_queue_unavailable` | — | EC |
